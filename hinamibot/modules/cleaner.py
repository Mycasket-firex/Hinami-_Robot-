...
async def clean_blue_text_must_click(update: Update,
    ...
    if isinstance(member, ChatMemberAdministrator):
        if ((member.can_delete_messages
             if isinstance(member, ChatMemberAdministrator) else None)
...
async def remove_bluetext_ignore(update: Update,
    ...
    if len(args) >= 1:
        ...
        if removed:
            reply = (
                "<b>{}</b> has been removed from bluetext cleaner ignore list."
                .format(args[0], ))
        else:
...
SET_CLEAN_BLUE_TEXT_HANDLER = CommandHandler("cleanblue",
                                             ...
                                             block=False)
ADD_CLEAN_BLUE_TEXT_HANDLER = CommandHandler("ignoreblue",
                                             add_bluetext_ignore,
                                             block=False)
...
LIST_CLEAN_BLUE_TEXT_HANDLER = CommandHandler("listblue",
                                              bluetext_ignore_list,
                                              block=False)
CLEAN_BLUE_TEXT_HANDLER = MessageHandler(filters.COMMAND
