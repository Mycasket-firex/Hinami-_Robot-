...
from hinamibot import (DEL_CMDS, DEV_USERS, DRAGONS, SUPPORT_CHAT, application)
...
def check_admin(permission: str = None,
                is_bot: bool = False,
                is_user: bool = False,
                is_both: bool = False,
    ...
    def wrapper(func):
        ...
        async def wrapped(update: Update, context: ContextTypes.DEFAULT_TYPE,
            ...
            if chat.type == ChatType.PRIVATE and not (only_dev or only_sudo
                                                      or only_owner):
                return await func(update, context, *args, **kwargs)

            bot_member = await chat.get_member(context.bot.id
                                               ) if is_bot or is_both else None
            user_member = await chat.get_member(
                user.id) if is_user or is_both else None

            if only_owner:
                if isinstance(user_member,
                              ChatMemberOwner) or user.id in DEV_USERS:
                    return await func(update, context, *args, **kwargs)
                else:
            ...
            if permission:
                ...
                if is_bot:
                    if (getattr(bot_member, permission) if isinstance(
                            bot_member, ChatMemberAdministrator) else False):
                        return await func(update, context, *args, **kwargs)
                    elif no_reply:
                        return
                    else:
                ...
                if is_user:
                    if isinstance(user_member, ChatMemberOwner):
                        return await func(update, context, *args, **kwargs)
                    elif (getattr(user_member, permission) if isinstance(
                            user_member, ChatMemberAdministrator) else False
...
async def is_user_admin(chat: Chat,
                        user_id: int,
                        member: ChatMember = None) -> bool:
    ...
    if not member:
        with THREAD_LOCK:
            ...
            try:
                ...
                try:
                    chat_admins = await application.bot.getChatAdministrators(
                        chat.id)
                except Forbidden:
                    return False
...
async def is_user_ban_protected(chat: Chat,
                                user_id: int,
                                member: ChatMember = None) -> bool:
    if (chat.type == "private" or user_id in DRAGONS or user_id in DEV_USERS
            or user_id in [777000, 1087968824
...
async def is_user_in_chat(chat: Chat, user_id: int) -> bool:
    member = await chat.get_member(user_id)
    return member.status in (ChatMemberStatus.LEFT,
