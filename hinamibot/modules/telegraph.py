"""
Telegraph module to upload text and image/video to telegra.ph via hinamibot
Author/Written - @Lostdir69
...
from hinamibot import TEMP_DOWNLOAD_LOC, application
...
async def telegraph(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    r = await telegrph.create_account(short_name="hinamibot")
    auth_url = r["auth_url"]
    ...
    msg = await message.reply_text("<code>Started telegraph module...</code>",
                                   parse_mode="html")
    ...
    if len(args) >= 1:
        if message.reply_to_message and not message.reply_to_message.forum_topic_created:
            ...
            if args[0] == "m":
                ...
                downloaded_file = file.download_to_drive(TEMP_DOWNLOAD_LOC +
                                                         "/" + file_name)
                await msg.edit_text("<code>Downloaded image/video</code>",
                                    parse_mode="html")
                ...
                response = telegrph.create_page(page_title, html_content=text)
            ...
            await msg.edit_text(
                "Haha! I know this trick so tag any image/video/text")
