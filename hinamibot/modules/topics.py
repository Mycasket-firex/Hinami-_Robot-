...
# from hinamibot.modules.sql.topics_sql import (del_action_topic,
#                                                get_action_topic,
#                                                set_action_topic)
    ...
    if chat.is_forum:
        if len(args) < 1:
            await message.reply_text(
                "You must give a name for the topic to create.")
        else:
            name = " ".join(args)
            try:
                ...
                await message.reply_text(
                    ...
                    if topic else "Something happened")
                await context.bot.sendMessage(
                    chat_id=chat.id,
                    text=
                    f"Congratulations {topic.name} created successfully\nID: {topic.message_thread_id}",
...
async def delete_topic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.is_forum:
        if len(args) > 0:
            try:
                topic_chat = await context.bot.delete_forum_topic(
                    chat.id, args[0])
                if topic_chat:
        ...
        await message.reply_text(
            "You can perform this in topics enabled groups only.")
        return ""
...
async def close_topic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.is_forum:
        if len(args) > 0:
            try:
                topic_chat = await context.bot.close_forum_topic(
                    chat.id, args[0])
                if topic_chat:
...
async def open_topic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.is_forum:
        if len(args) > 0:
            try:
                topic_chat = await context.bot.reopen_forum_topic(
                    chat.id, args[0])
                if topic_chat:
