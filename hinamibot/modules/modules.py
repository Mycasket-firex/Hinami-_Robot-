...
from hinamibot import application, telethn
from hinamibot.__main__ import (
    CHAT_SETTINGS,
    DATA_EXPORT,
    DATA_IMPORT,
    HELPABLE,
...
async def load(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        imported_module = importlib.import_module("hinamibot.modules." + text)
    except:
...
async def listmodules(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for helpable_module in HELPABLE:
        ...
        file_name = file_info.__name__.rsplit("hinamibot.modules.", 1)[1]
