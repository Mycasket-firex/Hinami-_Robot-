...
from hinamibot.modules.helper_funcs.handlers import CMD_STARTERS, SpamChecker
from hinamibot.modules.helper_funcs.misc import is_module_loaded
...
if is_module_loaded(FILENAME):
    ...
    from hinamibot.modules.sql import disable_sql as sql
    ...
    class DisableAbleCommandHandler(CommandHandler):
        ...
        def check_update(
            self, update
        ) -> Optional[Union[bool, Tuple[List[str], Optional[Union[bool,
                                                                  Dict]]]]]:
                        ...
                        if filter_result:
                            ...
                            if sql.is_command_disabled(
                                ...
                                is_disabled = command_parts[
                                    0] in ADMIN_CMDS and asyncio.ensure_future(
    ...
    @connection_status
    @check_admin(is_user=True)
    async def disable_module(update: Update,
                             context: ContextTypes.DEFAULT_TYPE):
        args = context.args
        chat = update.effective_chat
        if len(args) >= 1:
            disable_module = "hinamibot.modules." + args[0].rsplit(".", 1)[0]
    ...
    @connection_status
    @check_admin(is_user=True)
    async def enable_module(update: Update,
                            context: ContextTypes.DEFAULT_TYPE):
        args = context.args
        chat = update.effective_chat

        if len(args) >= 1:
            enable_module = "hinamibot.modules." + args[0].rsplit(".", 1)[0]
    ...
    DISABLE_MODULE_HANDLER = CommandHandler("disablemodule",
                                            disable_module,
                                            block=False)
    ENABLE_HANDLER = CommandHandler("enable", enable, block=False)
    ENABLE_MODULE_HANDLER = CommandHandler("enablemodule",
                                           enable_module,
                                           block=False)
