...
import hinamibot.modules.sql.welcome_sql as sql
import hinamibot
...
from hinamibot.modules.helper_funcs.misc import build_keyboard, revert_buttons
from hinamibot.modules.helper_funcs.msg_types import get_welcome_type
from hinamibot.modules.helper_funcs.string_handling import (
    escape_invalid_curly_brackets,
    markdown_parser,
    markdown_to_html,
)
from hinamibot.modules.log_channel import loggable
from hinamibot.modules.sql.global_bans_sql import is_user_gbanned
...
# from hinamibot.modules.sql.topics_sql import get_action_topic
...
async def send(update: Update, message, keyboard, backup_message):
    ...
    if cleanserv:
        try:
            await application.bot.delete_message(chat.id,
...
async def new_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for new_mem in new_members:
        ...
        if new_mem.id == bot.id and not hinamibot.ALLOW_CHATS:
        ...
        if should_welc:
            ...
            # Welcome yourself
                ...
                creator = None
                for x in await bot.get_chat_administrators(
        ...
        if (await is_user_ban_protected(chat, new_mem.id, await
...
async def welcome(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not args or args[0].lower() == "noformat":
        ...
        if welcome_type == sql.Types.BUTTON_TEXT or welcome_type == sql.Types.TEXT:
            ...
            if noformat:
                welcome_m += revert_buttons(buttons)
                await ENUM_FUNC_MAP[welcome_type](chat.id,
                                                  cust_content,
                                                  caption=welcome_m)
...
async def welcome_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    WELC_HELP_TXT = (
        ...
        "buttons.\n"
        f"To create a button linking to your rules, use this: `[Rules](buttonurl://t.me/{context.bot.username}?start=group_id)`. "
...
NEW_MEM_HANDLER = MessageHandler(filters.StatusUpdate.NEW_CHAT_MEMBERS,
                                 new_member,
                                 allow_edit=True,
                                 block=False)
LEFT_MEM_HANDLER = MessageHandler(filters.StatusUpdate.LEFT_CHAT_MEMBER,
                                  left_member,
                                  allow_edit=True,
                                  block=False)
...
WELCOME_MUTE_HELP = CommandHandler("welcomemutehelp",
                                   welcome_mute_help,
                                   block=False)
