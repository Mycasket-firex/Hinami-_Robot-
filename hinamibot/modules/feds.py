...
import hinamibot.modules.sql.feds_sql as sql
...
from hinamibot.modules.helper_funcs.chat_status import is_user_admin
from hinamibot.modules.helper_funcs.extraction import (
    extract_unt_fedban,
    extract_user,
    extract_user_fban,
)
from hinamibot.modules.helper_funcs.string_handling import markdown_parser
            ...
            if get_fedlog:
                if ast.literal_eval(get_fedlog):
                    await bot.send_message(
                        ...
                        parse_mode="markdown",
                        message_thread_id=update.effective_message.
                        message_thread_id if chat.is_forum else None)
            await send_message(
                update.effective_message,
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "hinami_fbanned_users.json"
                await update.effective_message.reply_document(
                    document=output,
                    filename="hinami_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "hinami_fbanned_users.csv"
                await update.effective_message.reply_document(
                    document=output,
                    filename="hinami_fbanned_users.csv",
...
async def fed_notif(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args:
        if args[0] in ("yes", "on"):
            ...
            await msg.reply_text("Please enter `on`/`off`",
                                 parse_mode="markdown")
    else:
...
FED_IMPORTBAN_HANDLER = CommandHandler("importfbans",
                                       fed_import_bans,
                                       block=False)
FEDSTAT_USER = DisableAbleCommandHandler(["fedstat", "fbanstat"],
                                         fed_stat_user,
                                         block=False)
...
DELETEBTN_FED_HANDLER = CallbackQueryHandler(del_fed_button,
                                             pattern=r"rmfed_",
                                             block=False)
FED_OWNER_HELP_HANDLER = CommandHandler("fedownerhelp",
                                        fed_owner_help,
                                        block=False)
FED_ADMIN_HELP_HANDLER = CommandHandler("fedadminhelp",
                                        fed_admin_help,
                                        block=False)
FED_USER_HELP_HANDLER = CommandHandler("feduserhelp",
                                       fed_user_help,
                                       block=False)
